@REM play-scala-seed launcher script
@REM
@REM Environment:
@REM JAVA_HOME - location of a JDK home dir (optional if java on path)
@REM CFG_OPTS  - JVM options (optional)
@REM Configuration:
@REM PLAY_SCALA_SEED_config.txt found in the PLAY_SCALA_SEED_HOME.
...
if "%PLAY_SCALA_SEED_HOME%"=="" (
  set "APP_HOME=%~dp0\\.."

  rem Also set the old env name for backwards compatibility
  set "PLAY_SCALA_SEED_HOME=%~dp0\\.."
) else (
  set "APP_HOME=%PLAY_SCALA_SEED_HOME%"
)
...
set "CFG_FILE=%APP_HOME%\PLAY_SCALA_SEED_config.txt"
set CFG_OPTS=
...
if "%JAVAOK%"=="false" (
  ...
  echo and download a valid Java JDK and install before running play-scala-seed.
  echo.
...
"%_JAVACMD%" !_JAVA_OPTS! !PLAY_SCALA_SEED_OPTS! -cp "%APP_CLASSPATH%" %MAIN_CLASS% !_APP_ARGS!
